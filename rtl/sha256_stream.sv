...
//
// One job    = one 64-byte message = BEATS_PER_BLOCK input beats  on s_axis.
// One result = one 32-byte digest  = BEATS_PER_HASH  output beats on m_axis,
//              with TLAST asserted ONLY on the final beat of the final
//              digest in a host burst (XDMA C2H semantics).
//
// Style: gearbox.  See docs/CODING_STYLE.md.  Only `assign` (combinational)
// and `always_ff` (one block per register).  No FSMs, no always_comb.
//
// Two "slots" in flight (matched to the engine's NUM_LANES=2):
//   slot.busy            : occupied
...
//   slot.is_last         : carries s_axis_tlast through to m_axis_tlast
//
`timescale 1ns/1ps
...
(
  ...
  // ===========================================================================
  // RX  --  collect BEATS_PER_BLOCK input beats into one BLOCK_W message reg
  // ===========================================================================
  ...
  logic rx_beat_last_in_msg_w;       // final beat of the 4-beat message
  ...
  // ---- rx_beat_cnt_q  (free-running 2-bit counter; naturally wraps) -------
  ...
  // ---- rx_msg_full_q -------------------------------------------------------
  // set = final-beat-of-message accepted; clear = engine consumed the buffer.
  logic rx_msg_full_set_w;
  logic rx_msg_full_clear_w;
  logic rx_msg_full_next;

  assign rx_msg_full_set_w   = rx_beat_last_in_msg_w;
  assign rx_msg_full_clear_w = rx_msg_consume_w;
  assign rx_msg_full_next    = rx_msg_full_set_w | (rx_msg_full_q & ~rx_msg_full_clear_w);
  ...
  // ---- rx_msg_is_last_q  (latch s_axis_tlast at message boundary) ---------
  // XDMA H2C asserts tlast on the final beat of each host write().  We
  // carry that flag through the slot all the way to m_axis_tlast so a
  // single host read() returns the entire burst in one descriptor (instead
  // of one descriptor per 32-byte digest, which back-pressures the engine).
  always_ff @(posedge aclk) begin
    if (rx_beat_last_in_msg_w) rx_msg_is_last_q <= s_axis_tlast;
  end

  // ===========================================================================
  // SLOT STATE  (NUM_LANES slots, one per compress-engine lane)
  // ===========================================================================
  ...
  logic [HASH_W-1:0] slot_cv_q            [0:NUM_LANES-1];   // intermediate CV
  logic [HASH_W-1:0] slot_digest_q        [0:NUM_LANES-1];   // final digest

  // ===========================================================================
  // ISSUE / DISPATCH SELECTION  (pure combinational)
  // ===========================================================================
  // Pick the FREE slot for new message admission (biased toward slot 0).
  logic empty_slot_w;
  logic have_empty_slot_w;
  assign empty_slot_w      =  slot_busy_q[0];
  assign have_empty_slot_w = ~slot_busy_q[0] | ~slot_busy_q[1];

  // Pick a slot ready for second-block dispatch (biased toward slot 0).
  ...
  // Always ready to consume results -- only NUM_LANES jobs ever in flight.
  ...
  // ===========================================================================
  // TX  --  emit captured digest as BEATS_PER_HASH beats on master
  // ===========================================================================
  logic                       tx_slot_w;
  logic                       have_tx_w;
  ...
  logic [BEAT_IDX_HASH_W-1:0]  tx_beat_cnt_q;   // 0 = high half, 1 = low half + tlast

  // Combinational handshake events
  logic tx_start_w;       // begin emitting a new digest
  logic tx_beat_tfer_w;   // master accepts the current beat
  ...
  // TLAST ONLY on the final beat of the FINAL digest of a host burst, so
  // the XDMA C2H descriptor isn't completed early on every 32-byte boundary.
  ...
  // ---- tx_active_q ---------------------------------------------------------
  // set = tx_start_w, clear = tx_last_beat_w.
  ...
  // ---- tx_active_slot_q  (latched at start) -------------------------------
  always_ff @(posedge aclk) begin
    if (tx_start_w) tx_active_slot_q <= tx_slot_w;
  end

  // ---- tx_beat_cnt_q -------------------------------------------------------
  // set = transferring beat 0 (advance to 1); clear = start or last beat.
  ...
  // ===========================================================================
  // All events that touch this slot are computed at the top of the block,
  // then each `_q` reg has its own set/clear/next/always_ff cluster directly
  // below.  Top-to-bottom you can see the entire life-cycle of one slot.
  for (genvar S = 0; S < NUM_LANES; S = S + 1) begin : g_slot
    ...
    // ---- slot_first_done_q -------------------------------------------------
    // set = first-CV captured; clear = slot reload OR emit-done.
    logic slot_first_done_clear_w;
    logic slot_first_done_next;
    assign slot_first_done_clear_w = slot_load_w | slot_emit_done_w;
    assign slot_first_done_next    =
        slot_capture_first_w
      | (slot_first_done_q[S] & ~slot_first_done_clear_w);
    ...
    // ---- slot_second_issued_q ----------------------------------------------
    logic slot_second_issued_clear_w;
    logic slot_second_issued_next;
    assign slot_second_issued_clear_w = slot_load_w | slot_emit_done_w;
    assign slot_second_issued_next    =
        slot_second_w
      | (slot_second_issued_q[S] & ~slot_second_issued_clear_w);
    ...
    // ---- slot_digest_ready_q -----------------------------------------------
    logic slot_digest_ready_clear_w;
    logic slot_digest_ready_next;
    assign slot_digest_ready_clear_w = slot_load_w | slot_emit_done_w;
    assign slot_digest_ready_next    =
        slot_capture_final_w
      | (slot_digest_ready_q[S] & ~slot_digest_ready_clear_w);
    ...
    // ---- slot_cv_q  (intermediate CV after first block) --------------------
    always_ff @(posedge aclk) begin
      if (slot_capture_first_w) slot_cv_q[S] <= eng_res_cv_out;
    end

    // ---- slot_digest_q  (final digest) -------------------------------------
    always_ff @(posedge aclk) begin
      if (slot_capture_final_w) slot_digest_q[S] <= eng_res_cv_out;
    end

    // ---- slot_is_last_q  (carries TLAST through to m_axis_tlast) -----------
    always_ff @(posedge aclk) begin
